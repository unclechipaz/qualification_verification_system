...
| FR-SRCH-11 | Empty verification input is handled; maximum length, malformed identifiers and API boundary cases are not comprehensively validated. | Invalid non-existent query test exists only at service level. | **P** | Artwell: add format/length validators and boundary tests. |
| FR-SRCH-12 | No DRF throttling or equivalent request-rate control is configured. | No rate-limit test. | **N** | Artwell + Charlton: configure scoped throttles and test 429 responses. |
